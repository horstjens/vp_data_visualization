...
class Sim:
    ...
    #radius = {  "generators": 0.05,
    #            "nodes": 4,
        #"pointer0": 2 + 0,  # for wind angle
        #"pointer1": 2 + 0,  # for wind angle
        #"geo1": grid_max_x / 2 - 25,
        #"geo2": grid_max_x / 2 - 1,
    #}
...
def create_stuff():
    ...
    for lon in range(int(Sim.x1), int(Sim.x2) + 1, 1):
        Sim.grid.append(vp.curve(pos=[vp.vector(lon, 0, geo_to_local(Sim.z1)),
                                 ...
                                 pickable=False,
                                 radius=0.0))
        Sim.labels["grid_legend"].append(vp.label(pos=vp.vector(lon, 0, geo_to_local(Sim.z1)), text=f"lon:{lon:.0f}", box=False, yoffset=-30, line=False,
                 color=vp.color.black))
