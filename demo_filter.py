...
class Sim:
    ...
    test_arrow = None
    canvas_width = 800 # 3d canvas
    canvas_height = 900 # 3d canvas
    dia_width = 365 #250     # for the 12 little diagrams on the right side
    dia_height = 180 #125    # for the 12 little diagrams on the right side
    dia_h_delta = 43 # correction for height of needle diagram
    mode = "arrange"
    camera_height = 0.25
    camera_range = 1.75
    camera_pitch = -90
...
def widget_func_create_curves(b):
    ...
    Sim.gui["filter_combo"].disabled = False
    Sim.gui["filter_apply"].disabled = False
...
def widget_func_filter_combo(b):
    ...
    match b.selected:
        case "Nodes":
            ...
            visible_node_names = [v for k,v in Data.node_names.items() if k in visible_node_numbers]
            Sim.gui["filter_text"].text = " ".join(visible_node_names)
        case "Generators":
            visible_node_numbers = [n for n in Sim.gui["generator_angle_curves"].keys() if Sim.gui["generator_angle_curves"][n].visible]
            visible_node_names = [v for k, v in Data.node_names.items() if k in visible_node_numbers]
            Sim.gui["filter_text"].text = " ".join(visible_node_names)

        case "Loads":
            visible_node_numbers = [n for n in Sim.gui["load_curves"].keys() if
                                    Sim.gui["load_curves"][n].visible]
            visible_node_names = [v for k, v in Data.node_names.items() if k in visible_node_numbers]
            Sim.gui["filter_text"].text = " ".join(visible_node_names)

        case "Storages":
            visible_node_numbers = [n for n in Sim.gui["storage_power_curves"].keys() if
                                    Sim.gui["storage_power_curves"][n].visible]
            visible_node_names = [v for k, v in Data.node_names.items() if k in visible_node_numbers]
            Sim.gui["filter_text"].text = " ".join(visible_node_names)

        case "Cables":
            visible_node_numbers = [n for n in Sim.gui["cable_loading_curves"].keys() if
                                    Sim.gui["cable_loading_curves"][n].visible]
            print("cables loading curves ", Sim.gui["cable_loading_curves"])
            print("visible nod numers:", visible_node_numbers)
            #visible_node_names = [k for k, v in Data.node_names.items() if k in visible_node_numbers]
            # node number is a string like "5_22"
            from_numbers = [int(n.split("_")[0]) for n in visible_node_numbers]
            to_numbers = [int(n.split("_")[1]) for n in visible_node_numbers]
            from_to = list(zip(from_numbers, to_numbers))
            from_to_text = [f"{Data.node_names[f]}_{Data.node_names[t]}" for (f,t) in from_to]
            #Sim.gui["filter_text"].text = " ".join(visible_node_numbers)
            Sim.gui["filter_text"].text = " ".join(from_to_text)


    # cables are different
    #visible_node_names = [v for k, v in Data.node_names.items() if k in visible_node_numbers]
    #Sim.gui["filter_text"].text = " ".join(visible_node_names)
    #print(b.selected, visible_node_numbers)
    #print(visible_node_names)

def widget_func_filter_apply(bu):
    """
       button
       Sim.gui["filter_text"]
       Data.node_names = {} # node_number: node_name
            ...
            for curvename in curvenames:
                if b.text.lower() in ("none", "0", "nothing", "clear"):
                    ...
                    for word in filterwords:
                        ...
                        elif (word.count("_") == 1) and word.split("_")[0].isdigit() and word.split("_")[1].isdigit():
                            filternumbers.append(word)
                        elif (word.count("_") == 1) and not word.split("_")[0].isdigit() and not word.split("_")[1].isdigit():
                            from_word, to_word = word.split("_")
                            filternumbers.append(f"{Data.node_names2[from_word]}_{Data.node_names2[to_word]}")
                    for node_number, curve in Sim.gui[curvename].items():
...
def widget_func_show_hide(b):
    """button"""
    # widgets have no visible attribute, but can be deleted
    #print(Sim.gui.items())
    if b.text == "hide gui elements":
        #Sim.gui["show_hide"].text = "show gui elements"
        b.text ="show gui elements"
        hide = True
    else:
        b.text = "hide gui elements"
        hide = False

    for name, widget in Sim.gui.items():
        # list of widgets that always stay visible
        # legend is a list
        if name in ("subnodes_add", "subnodes_remove", "mode", "simulation", "create_curves", "show_hide",
                    "restart", "step_back", "play", "step_forward", "end",
                    "timeframe", "frameslider", "label_frame", "quit",
                    "legend",
                    "legend_combo","filter_combo","filter_text","filter_apply",
                    ):
            continue
        if "curves" in name:
            continue
        if hide:
            ...
            pass
            #widget.visible = True
            #Sim.gui["show_hide"].text = "hide gui elements"
        #print(name, widget.visible)
    #if hide:
    #    Sim.scene.caption = ""
    Sim.scene3.caption = ""
    print("title is now:", Sim.scene.title)
    print("caption is now:", Sim.scene.caption)
    print("caption scene3 is now:", Sim.scene3.caption)
    print("caption anchor is now:", Sim.scene.caption_anchor)
...
def widget_func_start_simulation(b):
    ...
    Sim.gui["subnodes_add"].delete()
    Sim.gui["subnodes_remove"].delete()
    Sim.gui["mode"].delete()
    Sim.gui["simulation"].delete()
...
def create_widgets():
    ...
    Sim.gui["create_curves"] = vp.button(pos=Sim.scene.title_anchor,
                                         ...
                                         text="create curves",
                                         disabled=True)
    #Sim.gui["show_hide"] = vp.button(pos=Sim.scene.title_anchor,
    #                                 bind=widget_func_show_hide,
    #                                 text="hide gui elements",
    #                                 )
    ...
    # was scene3 before, is now scene
    Sim.scene.append_to_caption("camera goto:")
    Sim.gui["goto"] = vp.menu(pos=Sim.scene.caption_anchor,
                               selected="center",
...
def create_stuff2_curves():
    ...
    for col_name in [name for name in Data.col_names if name.startswith("cable_loading_")]:
        node_number2 = int(col_name.split("_")[-1])
        node_number1 = int(col_name.split("_")[-2])
        print("cable colname:", col_name, node_number1, node_number2)
        gdata = list(zip(Data.df["time"], Data.df[col_name]))
        Sim.gui["cable_loading_curves"][f"{node_number1}_{node_number2}"] = vp.gcurve(graph=Sim.graph_dia8,
                                                                     color=Sim.legend_nodes[node_number1],
                                                                     data=gdata)
    ...
    for col_name in [name for name in Data.col_names if name.startswith("cable_power_")]:
        ...
        #print("cable colname:", col_name, node_number1, node_number2)
        gdata = list(zip(Data.df["time"], Data.df[col_name]))
        Sim.gui["cable_power_curves"][f"{node_number1}_{node_number2}"] = vp.gcurve(graph=Sim.graph_dia9,
        color = Sim.legend_nodes[node_number1],
        data = gdata)
...
def create_stuff2():
    ...
    Sim.scene.append_to_caption(" Show in diagrams: ")
    ...
    Sim.gui["filter_text"] = vp.winput(pos=Sim.scene.caption_anchor,
                                       ...
                                       type="string",
                                       width=Sim.dia_width*1,  # because 2 columns of diagrams at the right
                                       text="all nodes",
                                       )
    Sim.gui["filter_apply"] = vp.button(pos=Sim.scene.caption_anchor, bind=widget_func_filter_apply, text="apply", disabled=True)
